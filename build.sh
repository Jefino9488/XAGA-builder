...
Start_Time
wget --limit-rate=20m -O "$GITHUB_WORKSPACE/recovery_rom.zip" "${URL}"
...
for i in mi_ext odm product system system_ext vendor vendor_dlkm; do
  echo -e "\e[1;31m - Generating: $i \e[0m"
  sudo python3 "$GITHUB_WORKSPACE"/tools/fspatch.py "$GITHUB_WORKSPACE"/super_maker/$i "$GITHUB_WORKSPACE"/super_maker/config/"$i"_fs_config
  sudo python3 "$GITHUB_WORKSPACE"/tools/contextpatch.py "$GITHUB_WORKSPACE"/super_maker/$i "$GITHUB_WORKSPACE"/super_maker/config/"$i"_file_contexts
  Start_Time
  sudo "$GITHUB_WORKSPACE"/tools/mkfs.erofs -zlz4hc,9 -T 1230768000 --mount-point /$i --fs-config-file "$GITHUB_WORKSPACE"/super_maker/config/"$i"_fs_config --file-contexts "$GITHUB_WORKSPACE"/super_maker/config/"$i"_file_contexts "$GITHUB_WORKSPACE"/super_maker/$i.img "$GITHUB_WORKSPACE"/super_maker/$i >/dev/null
  End_Time packagingErofs
  eval "${i}_size=\$(du -sb \"$GITHUB_WORKSPACE/super_maker/$i.img\" | awk '{print \$1}')"
  sudo rm -rf "$GITHUB_WORKSPACE"/super_maker/$i
done
...
mv "$GITHUB_WORKSPACE/images/boot.img" "$GITHUB_WORKSPACE/${device}/boot/"

mv "$GITHUB_WORKSPACE/images/vendor_boot.img" "$GITHUB_WORKSPACE/${device}/twrp/"
