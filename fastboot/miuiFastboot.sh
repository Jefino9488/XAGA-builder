...
for cmd in aria2c 7z payload-dumper-go lpmake du realpath xargs timeout comm diff rsync shasum jq; do
    if ! command -v "$cmd" >/dev/null 2>&1; then
        echo -e "${color_red}Error: command '$cmd' not found${color_normal}"
        exit 1
    fi
done

# Check for required arguments
if [ -z "$url" ] || [ -z "$github_workspace" ] || [ -z "$device" ] || [ -z "$key" ]; then
    echo -e "${color_red}Error: missing required argument${color_normal}"
    exit 1
fi
...
# Define temporary directory names
tmp_dir="$(mktemp -d)"
...
function success() {
    log "success" "$1"
    echo -e "${green}Success: $1${normal}"
}

# Download package
info "- Downloading package"
aria2c -x16 -j"$(nproc)" -U "Mozilla/5.0" -d "$github_workspace" -o "recovery_rom.zip" "$url" || exit
success "- Downloaded recovery rom"

# Set permissions and create directories
chmod -R 777 "$github_workspace/tools"
...
7z x "$github_workspace/$recovery_zip" -o"$github_workspace/$device" payload.bin || exit
rm -rf "$github_workspace/$recovery_zip"

# Extract images from payload.bin
mkdir -p "$github_workspace/$device/images"
"$github_workspace/tools/payload-dumper-go" -o "$github_workspace/$device/images" "$github_workspace/$device/payload.bin" >/dev/null || exit
...
for img in vendor product system system_ext odm_dlkm odm mi_ext vendor_dlkm; do
    mv "$github_workspace/$device/images/$img.img" "$github_workspace/super_maker/" || exit
...
--group dynamic_partitions_b:9126805504 --virtual-ab --output "$github_workspace/super_maker/super.img" || exit

# Move super.img to images directory
mv "$github_workspace/super_maker/super.img" "$github_workspace/$device/images/" || exit
success "- Moved super.img"
...
# Patch boot image
info "- Patching boot image"
cp "$github_workspace/$device/images/boot.img" "$github_workspace/$device/boot/" || exit

if ! "$github_workspace/magisk/boot_patch.sh" "$github_workspace/$device/boot/boot.img"; then
    error "- Failed to patch boot image"
fi

success "- Patched boot image"

# Move patched boot image to boot directory
mv "$github_workspace/magisk/new-boot.img" "$github_workspace/$device/boot/magisk_boot.img" || exit

# Move original boot and vendor_boot images to their respective directories
mv "$github_workspace/$device/images/boot.img" "$github_workspace/$device/boot/" || exit
mv "$github_workspace/$device/images/vendor_boot.img" "$github_workspace/$device/vendor_boot/" || exit

# Copy flasher.exe to device directory
cp "$github_workspace/tools/flasher.exe" "$github_workspace/$device/" || exit

# Copy preloader_ari.bin to images directory
if [ -f "$github_workspace/tools/preloader_ari.bin" ]; then
    cp "$github_workspace/tools/preloader_ari.bin" "$github_workspace/$device/images/" || exit
    success "preloader_ari.bin copied successfully"
else
    warn "Failed to copy preloader_ari.bin"
fi

# Create zip file
cd "$github_workspace" || exit
zip -r "zip/$device\_fastboot.zip" "$device" || exit

success "- $device\_fastboot.zip created successfully"
